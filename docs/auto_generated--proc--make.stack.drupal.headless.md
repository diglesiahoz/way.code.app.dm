### make.stack.drupal.headless

```yml
help: Crear nuevo sitio Drupal (desacoplado)
example:
 - (({}.tmp.proc.sig))
task:
  require:
  ...
  do:
    ...
    - { event: 'origin windup' }
```
[```config/proc/make.stack.drupal.headless.yml```](../config/proc/make.stack.drupal.headless.yml)
