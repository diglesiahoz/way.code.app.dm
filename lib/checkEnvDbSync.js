    ...
    setTimeout(() => {
      (async function() {
        ...
        switch (origin._env) {
          ...
          case 'dev':
          case 'test':
            var origin_primary_env_color = `magenta`;
            break;
          case 'prod':
        ...
        switch (target._env) {
          case 'local':
            ...
            //var target_secondary_env_color = `black`;
            break;
          case 'pre':
          case 'dev':
          case 'test':
            var target_primary_env_color = `magenta`;
            //var target_secondary_env_color = `white`;
            break;
          case 'prod':
        ...
        var origin_sig = `color.${origin_primary_env_color}("${origin.appsetting.service.db.host.sv}/${origin.appsetting.service.db.name}")`;
        var target_sig = `color.${target_primary_env_color}("${target.appsetting.service.db.host.sv}/${target.appsetting.service.db.name}")`;
        ...
        var sync_status = `color.bold.${target_primary_env_color}(" SYNC ")`;
        var sync_arrow_begin = `color.bold.${target_primary_env_color}("===")`;
        var sync_arrow_end = `color.bold.${target_primary_env_color}("===>")`;

        console.log(`${eval(origin_sig)} ${eval(sync_arrow_begin)}${eval(sync_status)}${eval(sync_arrow_end)} ${eval(target_sig)}`);
        if (target._env != 'local') {
          way.opt.y = false;
        }
        await way.lib.ask({
