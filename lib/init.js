    ...
    setTimeout(() => {
      (async function() {
        ...
        if (_args.name !== "" && stack_found) {
          ...
          for (service of Object.keys(envThis.appsetting.service)) {
            try {
              if (fs.existsSync(`${way.root_app}/file/stack/${_args.name}/.dm/common/service.${service}.yml`)){
                var service_data = fs.readFileSync(`${way.root_app}/file/stack/${_args.name}/.dm/common/service.${service}.yml`, 'utf8');
              ...
              if (fs.existsSync(`${way.root_app}/file/stack/${_args.name}/.dm/common/volume.${service}.yml`)){
                var volume_data = fs.readFileSync(`${way.root_app}/file/stack/${_args.name}/.dm/common/volume.${service}.yml`, 'utf8');
                volume_data = await way.lib.decode({ data: volume_data }).catch((o) => { return {} });
                var parsed = '';
                volume_data.split(/\n/).forEach(line => {
                  parsed = `${parsed}  ${line}\n`;
                });
                volumes[service] = parsed;
              }
            } catch (e) { }
          }
          //console.log(services);way.lib.exit();
          //console.log(volumes);way.lib.exit();
          var data = `networks:\n  dm_proxy:\n    external: true\n`;
          // Comprueba si ha de establecer volumes
          if (!way.lib.check(volumes)) {
            var data = `${data}\nvolumes: {}\n`;
          } else {
            var data = `${data}\nvolumes:\n`;
            for (service of Object.keys(services)) {
              if (way.lib.check(volumes[service])) {
                data = `${data}${volumes[service]}`;
              }
            }
          }
          if (!way.lib.check(services)) {
            var data = `${data}\nservices: {}\n`;
          } else {
            var data = `${data}\nservices:\n`;
            for (service of Object.keys(services)) {
              if (way.lib.check(services[service])) {
                data = `${data}${services[service]}`;
              }
            }
          }
          //console.log(data);way.lib.exit()
          var target = `${envThis.appsetting.root}/.dm/docker-compose.yml`;
          fs.writeFileSync(target, `${data}`);
          fs.chown(target, way.user.uid, way.user.gid, (error) => { /*way.lib.exit(error)*/ });
          way.lib.log({ message:`Created compose file: ${target}`, type:`label`});
          
          
          console.log('INIT!!')
          console.log('stack drupal_new')
          way.lib.exit()
