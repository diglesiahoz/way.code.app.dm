  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      (async function() {
        ...
        //way.var.service_www_env = '{ APPSETTING_USER: "${APPSETTING_USER}", APPSETTING_UID: "${APPSETTING_UID}", APPSETTING_GID: "${APPSETTING_GID}", ';
        way.var.service_www_env = '{';
        var env_key_parsed = [];
        var to_run = way.var.loadEnv;
        to_run.forEach(env_var => {
          env_var_key = `${env_var.split('=')[0]}`;
          if (!env_key_parsed.includes(env_var_key)) {
            env_key_parsed.push(env_var_key);
            way.var.service_www_env = (way.lib.check(way.var.service_www_env)) ? `${way.var.service_www_env} ${env_var_key}: "\${${env_var_key}}",` : `${env_var_key}: "\${${env_var_key}}",`;
          }
        });
        way.var.service_www_env = `${ way.var.service_www_env.replace(/,$/, "") } }`;
        //way.lib.exit()
