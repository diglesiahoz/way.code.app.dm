  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      (async function() {
        ...
        const BINARY_DEPLOY_EXT = new Set([
          '.jpg', '.jpeg', '.png', '.gif', '.webp', '.ico', '.bmp', '.tif', '.tiff', '.avif',
        ...
        const isBinaryDeployFile = (filePath) => {
          const ext = path.extname(filePath).toLowerCase();
          if (BINARY_DEPLOY_EXT.has(ext)) {
            return true;
          }
          let fd;
          try {
        ...
        */
        ...
        for (key of Object.keys(way.map.file)) {
          var stack_to_check = _args.name.split('-')[0];
          var re = new RegExp(`app\\.${app_name_root}\\.stack\\.${stack_to_check}\\.`, "i");
          if (re.test(key)) {
            stack_found = true;
          }
        }
          ...
          for (abs_path of paths) {
            ...
            if (flag_include) {
              ...
              if (fs.lstatSync(abs_path).isDirectory()) {
                ...
                const deployBinary = isBinaryDeployFile(abs_path);
                var file_data;
                var data;
                ...
                if (deployBinary) {
                  ...
                  file_data = fs.readFileSync(abs_path, 'utf8');
                ...
                if (fs.existsSync(target)){
                  var current_file_data = deployBinary
