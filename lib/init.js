    ...
    setTimeout(() => {
      (async function() {
        ...
        if (_args.name !== "" && stack_found) {
          ...
          for (service of Object.keys(envThis.appsetting.service)) {
            try {
              if (fs.existsSync(`${way.root_app}/file/stack/${_args.name}/.dm/templates/docker.compose.service.${service}.yml`)){
                var service_data = fs.readFileSync(`${way.root_app}/file/stack/${_args.name}/.dm/templates/docker.compose.service.${service}.yml`, 'utf8');
              ...
              if (fs.existsSync(`${way.root_app}/file/stack/${_args.name}/.dm/templates/docker.compose.volume.${service}.yml`)){
                var volume_data = fs.readFileSync(`${way.root_app}/file/stack/${_args.name}/.dm/templates/docker.compose.volume.${service}.yml`, 'utf8');
