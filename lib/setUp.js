    ...
    setTimeout(() => {
      (async function() {
        ...
        if (Object.keys(envThis).length === 0) {
          way.lib.exit(`Required configuration "appsetting.root"`)
        }
        ...
        var parent_dir_name = envThis.appsetting.root;
        if (basename != ".dm") {
        ...
        way.var.setup_compose_cmd = `[ ! -f ${parent_dir_name}/.env ] && touch ${parent_dir_name}/.env; export COMPOSE_BAKE=true; ${way.var.setup_compose_cmd} up ${extra_opt} --force-recreate --build && echo && docker ps --filter name=^/(({origin}.appsetting.tag)) && echo`;
