  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      (async function() {
        ...
        // Por defecto levantamos rapido (sin build/recreate).
        // Si se pasa -f, forzamos reconstruccion/recreacion.
        var compose_up_opts = `up --remove-orphans ${extra_opt}`;
        if (way.opt.f) {
          compose_up_opts = `up --build --force-recreate --remove-orphans ${extra_opt}`;
        }
        way.var.setup_compose_cmd = `[ ! -f ../.env ] && touch ../.env; export COMPOSE_BAKE=true;${way.var.setup_compose_cmd} ${compose_up_opts}`;
