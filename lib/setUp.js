  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      (async function() {
        ...
        if (fs.existsSync(`${envThis.appsetting.root}/docker-compose.common.${envThis._env}.yml`)){
          way.var.setup_compose_cmd = `${way.var.setup_compose_cmd} -f docker-compose.common.${envThis._env}.yml`;
        }
        if (fs.existsSync(`${envThis.appsetting.root}/docker-compose.app.${envThis._env}.yml`)){
          way.var.setup_compose_cmd = `${way.var.setup_compose_cmd} -f docker-compose.app.${envThis._env}.yml`;
        }
        way.var.setup_compose_cmd = `${way.var.setup_compose_cmd} up -d --build && echo && docker ps --filter name=^/(({origin}.appsetting.tag)) && echo`;
        way.var.setup_compose_cmd = await way.lib.decode({ data: way.var.setup_compose_cmd }).catch((o) => { return {} });
        //console.log(way.var.setup_compose_cmd);
