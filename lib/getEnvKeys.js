  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      (async function() {
        ...
        //console.log(`To check: ${envThis._tag}`);

        let profile_keys = way.map.profileKey.filter((element) => {
          if (new RegExp(`^@${envThis._tag}\..*`,"g").test(element) && ! new RegExp(`_`,"g").test(element)) {
            return element;
          }
        });
        ...
        for (profile_key of profile_keys) {
          var env_key = profile_key.replace(new RegExp(`^@${envThis._tag}\.`,"g"), '');
        ...
        for (env_key of tmp_envs) {
          for (env_key_target of tmp_envs) {
            ...
            if (!new RegExp(`local$`,"g").test(env_key) && !new RegExp(`local$`,"g").test(env_key_target)) {
              ...
              //console.log(`@${envThis._tag}.${env_key}`, `@${envThis._tag}.${env_key_target}`);
              
              var to_load = [];
              if (!way.lib.check(envs[`@${envThis._tag}.${env_key}`])) {
                to_load.push(`@${envThis._tag}.${env_key}`);
              }
              if (!way.lib.check(envs[`@${envThis._tag}.${env_key_target}`])) {
                to_load.push(`@${envThis._tag}.${env_key_target}`);
              }
              if (to_load.length > 0) {
              ...
              // Comprueba etiquetas si el procedimiento es solo "dm.sync"
              if (way.proc.name == 'dm.sync') {
                var source_tags = envs[`@${envThis._tag}.${env_key}`].tags;
                var target_tags = envs[`@${envThis._tag}.${env_key_target}`].tags;
                if (way.lib.check(source_tags) && way.lib.check(target_tags)) {
                  ...
                  if (common_tags.length > 0) {
                    //console.log(`@${envThis._tag}.${env_key}`, source_tags);
                    //console.log(`@${envThis._tag}.${env_key_target}`, target_tags)
