    ...
    setTimeout(() => {
      (async function() {
        ...
        if (_args.excluded_tables.length > 0) {
          ...
          //var docker_exec = `${docker_cmd_source} ${way.var.db_dump_exec} ${db_access} ${db_query} | sed -e "s/^Tables.*//" | xargs -n1`;
        ...
        if (way.lib.check(_args.leap_from)) {
          way.var.target_database = (way.opt.d) ? `/tmp/file.sql.gz` : `/tmp/${filename}`;
        } else {
          way.var.target_database = (way.opt.d) ? `${_args.backup_db_path}/file.sql.gz` : `${_args.backup_db_path}/${filename}`;
        }
        ...
        if (way.lib.check(out.buffer)) {
          ...
          var out = await way.lib.exec({ cmd: `rm ${way.var.target_database}`, out: false });
        } else {
          ...
          if (way.lib.check(_args.leap_from)) {
            ...
            way.lib.clearLogRunning();
            if (way.lib.check(out.buffer)) {
              way.lib.log({ message: `Failed to sync database from "${_args.env_key}:${_args.host}/${_args.name}:${way.var.origin_database}" to "${way.var.target_database}"`, type: `warning` });
            } else {
              way.lib.log({ message: `Synchronized database from "${_args.env_key}:${_args.host}/${_args.name}:${way.var.origin_database}" to "${way.var.target_database}"`, type: `success` });
              // Elimina base de datos exportada en remoto.
              var exec = {};
              exec['out'] = true;
              exec['cmd'] = `rm ${way.var.origin_database}`;
              var exec = Object.assign(exec, _args.leap_from);
              way.lib.logRunning(`Deleting database from "${_args.env_key}:${_args.host}/${_args.name}" to "${way.var.target_database}"`);
              var out = await way.lib.exec(exec);
              way.lib.clearLogRunning();
              if (way.lib.check(out.buffer)) {
                way.lib.log({ message: `Could not remove database from "${_args.env_key}:${_args.host}/${_args.name}:${way.var.origin_database}"`, type: `warning` });
              } else {
                way.lib.log({ message: `Removed database from "${_args.env_key}:${_args.host}/${_args.name}:${way.var.origin_database}"`, type: `success` });
              }
            }
          }
        }
        ...
        return resolve({
          ...
          code: 0,
          data: {
            file: (way.opt.d) ? `file.sql.gz` : `${filename}`,
          },
        });
