  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      (async function() {
        ...
        // Ejemplos opciones: --set-gtid-purged=OFF --skip-triggers --lock-tables=false
        var db_query = `--quick --lock-tables=false --max_allowed_packet=512M ${_args.name} ${way.var.excluded_tables}`;
        ...
        way.lib.logRunning(`Exporting database from "${_args.host}/${_args.name}"`);
        ...
        if (way.lib.check(out.buffer)) {
          ...
          way.lib.log({ message: `Exported database from "${_args.host}/${_args.name}"`, type: `success` });
        }
