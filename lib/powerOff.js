    ...
    setTimeout(() => {
      (async function() {
        ...
        var containers_by_proxy = await way.lib.exec({
          cmd: `docker network inspect dmproxy | jq .[].Containers | jq "keys" | jq -rc .[]`,
          out: false,
          cast: true
        }).catch((o) => {
          return {};
        ...
        for (profileKey of way.map.profileKey) {
          ...
          if (typeof config.appsetting !== "undefined" && typeof config.appsetting == "object") {
            if (tags.includes(config._tag) == false) {
              tags.push(config._tag);
            }
          }
        }
        ...
        if (networks.buffer.length > 0 && networks.buffer.includes('dmproxy')) {
          way.lib.logRunning(`Turning off "dmproxy" network`);
          var out = await way.lib.exec({
            cmd: `docker network disconnect -f dmproxy ${containers_by_proxy.join(' ')}`,
            out: false,
            cast: true
          }).catch((o) => {
            return {};
          ...
          way.lib.clearLogRunning();
          way.lib.logRunning(`Deleting "dmproxy" network`);
          var out = await way.lib.exec({
            cmd: `docker network rm dmproxy`,
            out: false,
            cast: true
          }).catch((o) => {
            return {};
