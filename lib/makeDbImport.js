  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      (async function() {
        ...
        // Comprueba opciones
        if (way.lib.check(_args.file) && way.lib.check(way.opt.last) && way.opt.last) {
          way.lib.log({ message: `The last file is not obtained if the --file option is set`, type: 'warning' });  
        }
        
        // Obtiene ficheros
        var dumps = {};
        await require('glob').sync(`${_args.backup_db_path}/*{.sql,.sql.gz}`, {
          dot: true,
          ignore: [],
        }).map(abs_path => {
        ...
        if (!way.lib.check(dumps) && Object.keys(dumps).length == 0) {
          way.lib.exit(`No se han encontrado bases de datos (.sql|.sql.gz) para importar ficheros desde "${_args.backup_db_path}"`);
        }
        var source_file = '';
        if (way.lib.check(_args.file)) {
          if (way.lib.check(dumps[_args.file])) {
            ...
            way.lib.log({ message: `Not found file "${_args.file}"`, type: `warning` });  
          }
        }
        if (!way.lib.check(source_file)) {
          if (way.lib.check(way.opt.last) && way.opt.last && !way.lib.check(_args.file)) {
            ...
            var choice = await way.lib.complete({
              ...
              message: `Selecciona fichero a importar...`
            });
        ...
        if (way.opt.truncate) {
          ...
          await way.lib.ask({
            message: `Desea vaciar todas las tablas de la base de datos "${_args.name}"?`,
            exitIfNegative: true
          });
          way.var.tables = '';
          ...
          way.var.tables.split('\n').forEach(table_name => {
            var db_query = `${_args.name} -e "truncate table ${table_name}"`;
            var docker_exec = `${docker_cmd_source} ${way.var.db_exec} ${db_access} ${db_query}`;
            bulk_cmd.push(`${docker_exec}`);
          });
          var out = await way.lib.exec({ cmd: `${bulk_cmd.join(' && ')}`, out: false });
          if (way.lib.check(out.buffer)) {
            way.lib.log({ message: `${out.buffer}`, type: `error` });
          }
        }
        ...
        way.lib.logRunning(`Importing database`);
        var out = await way.lib.exec({ cmd: `${docker_exec}`, out: false });
        ...
        if (way.lib.check(out.buffer)) {
          ...
          way.lib.log({ message: `Imported database into "${_args.host}/${_args.name}"`, type: `success` });
        }
